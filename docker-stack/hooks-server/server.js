...
const server = http.createServer((req, res) => {
  ...
  req.on("data", (chunk) => {
    ...
    if (bodySize > MAX_BODY_BYTES) {
      ...
      res.end(JSON.stringify({ error: "Body too large", limit: MAX_BODY_BYTES }));
      return;
    }
  ...
  req.on("end", () => {
    ...
    try {
      ...
      console.log(`  #${seq} ERROR: Could not parse JSON body`);
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (hookEvent === "SessionStart") {
      console.log(`  [DEBUG SessionStart] Raw keys: ${JSON.stringify(Object.keys(parsed))}`);
      console.log(`  [DEBUG SessionStart] source=${parsed.source}, model=${parsed.model}`);
      console.log(`  [DEBUG SessionStart] Full payload: ${JSON.stringify(parsed, null, 2).slice(0, 2000)}`);
    }
...
server.listen(PORT, () => {
  ...
  console.log("");
  console.log(
    "  Configure hooks in .claude/settings.local.json (run /init-claude-analytics):",
  );
  console.log('    "hooks": {');
