...
CREATE MATERIALIZED VIEW IF NOT EXISTS claude_analytics.sessions_mv
TO claude_analytics.sessions
AS SELECT
...
async function tableExists(tableName) {
  const result = await chQuery(
    `EXISTS TABLE claude_analytics.${tableName}`,
  );
  return result === "1";
}
...
async function ensureSchemaVersionTable() {
  await chQuery(`
    CREATE TABLE IF NOT EXISTS claude_analytics.schema_version (
      version UInt32,
      name String,
...
async function getSchemaVersion() {
  const result = await chQuery(
    "SELECT max(version) FROM claude_analytics.schema_version",
  );
...
async function recordMigration(version, name, description) {
  await chQuery(
    `INSERT INTO claude_analytics.schema_version (version, name, description) VALUES (${version}, '${name}', '${description}')`,
  );
}
