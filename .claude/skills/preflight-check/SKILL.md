...
```bash
docker ps --filter "name=claude-analytics" --format "{{.Names}}" 2>/dev/null
```

If any of `claude-analytics-clickhouse`, `claude-analytics-otel`, `claude-analytics-hooks`, `claude-analytics-grafana` appear, mark those ports as "held by us" and skip the conflict check for them. If all 4 are running, report "Stack already running" and stop.
...
====================================
  ...
  Required Host Ports
  ───────────────────
    13000   Grafana UI                       [AVAILABLE / IN USE by <process> (PID <pid>) / held by claude-analytics-grafana]
