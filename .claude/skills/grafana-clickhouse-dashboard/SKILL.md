...
Hard-won lessons from debugging the Claude Analytics OTel dashboard. Apply these rules when writing or editing Grafana dashboard JSON that uses the `grafana-clickhouse-datasource` plugin.
...
SELECT DISTINCT mapKeys(ResourceAttributes) FROM claude_analytics.otel_logs LIMIT 1

-- Check LogAttributes keys
SELECT DISTINCT mapKeys(LogAttributes) FROM claude_analytics.otel_logs LIMIT 1

-- Check actual values for a specific key
SELECT ResourceAttributes['project.name'], count() FROM claude_analytics.otel_logs GROUP BY 1
```
...
```bash
MSYS_NO_PATHCONV=1 docker exec claude-analytics-clickhouse clickhouse-client --query "YOUR QUERY HERE"
```
...
{
  "refId": "A",
  "rawSql": "SELECT count() AS total FROM claude_analytics.otel_logs WHERE $__timeFilter(Timestamp)",
  "format": 1,
  "queryType": "sql",
  "datasource": {
