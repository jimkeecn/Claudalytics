...
[安装](#安装) · [功能](#功能) · [更新](#更新) · [团队使用](#团队使用) · [更新日志](../CHANGELOG.md)
...
**本地开发** — 如果你正在迭代插件代码本身，请改用本地检出目录进行安装：
...
导航至：**Home > Dashboards > Claudalytics > Claudalytics - OTel Overview**
...
查看 [CHANGELOG.md](../CHANGELOG.md) 了解每个版本的变更列表。
...
增量式数据库变更（新表、新物化视图）由 hooks-server 在启动时自动应用。如果某个版本包含破坏性的数据库变更（列类型修改、重新分区），请在 Claudalytics 项目中运行 `/migrate-db` — 它会引导你完成安全的并行迁移，并提示备份。

Hook 脚本和 hook 声明现在随插件一起分发，因此 `git pull` + 重新加载插件即可获取 hook 相关的更新。如果你从 1.0.0 升级，请在每个项目中重新运行一次 `/init-claudalytics` — 它会清理 `.claude/hooks/` 中遗留的按项目安装的 hook 脚本，并从 `.claude/settings.local.json` 中移除过时的 hook 条目。OTel 环境变量和项目名称都会被保留。
...
1. **部署到共享服务器** — Docker 服务栈可以在任何服务器上运行。每个开发者将 OTel 端点以及 `plugin/hooks/forward-hook.sh` 中的 `HOOKS_URL` 指向服务器地址，而不是 `localhost`。
2. **添加团队名称属性** — 在 `OTEL_RESOURCE_ATTRIBUTES` 中与 `project.name` 一起添加 `team.name`。
3. **从 hook 脚本转发 `team.name`** — 扩展 `plugin/hooks/forward-hook.sh`，在 hooks URL 中追加 `&teamName=<TEAM>`，并更新 hooks 服务器以记录该字段。
4. **更新 ClickHouse 表** — 在目标表和物化视图中添加 `team_name` 列。
5. **更新 Grafana** — 添加团队下拉变量，并使所有面板按该变量进行过滤。

**在部署到服务器之前，必须确保服务栈的安全性：**

- 设置 ClickHouse 密码（默认配置没有身份验证）
- 更改 Grafana 管理员密码
- 修改 `docker-compose.yaml` 中的主机端口绑定：将 `127.0.0.1:PORT:PORT` 改为 `PORT:PORT`（或特定网络接口），以便团队成员可以访问服务栈 — 默认绑定仅限 localhost，这是为了保护个人开发者在公共 WiFi 下的安全
- 使用防火墙限制端口访问 — 仅暴露端口 4317（OTel gRPC）、4319（hooks）和 13000（Grafana）
- 添加 TLS 以加密传输
